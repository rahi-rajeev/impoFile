(function(){
	...
	***/
	function productCtrl($scope,salesfactoryData,$http,$sce,$timeout,$filter, treeNodeService){
		...
		//console.log(product_detail.has_product_image.image);
		//console.log(product_detail.image_url + product_detail.has_product_image.image);
		img.src=product_detail.image_url + product_detail.has_product_image[0].image;
		//console.log(product_detail.has_product_image);
		//$scope.product.file_interface= product_detail.has_product_image;
		//console.log($scope.product.file_interface);
		...
		var selectable = true; 
		//get sepecifiaction data from server
		...
		function _getCatogary(){
			salesfactoryData.getData()
			.then(()=>{
				$scope.prdData.cateTree=rs[2].data; console.log(JSON.stringify($scope.prdData.cateTree));
			});
		};
		...
		function _getWarehouse(){
 		...
 		function _getReqmtSpeData(){
 			let urlarr =[attribute_requirment_url,warehoseurl,categorylisturl];
 			salesfactoryData.getAllData(urlarr,'GET',{action : "productCatEdit"}).then((rs)=>{
 				//attrbute section
