...
(function(){
	...
	*@param : prd_attr_selection {array} ->store data main product wise
	*/
	var prd_attr_selection = [];

	function prdSelectionData(obj, prd_type){
		if(obj!== undefined && obj){
			_.map(obj, function(item, key){
				//in case of config nad bundel
				if(!_.isArray(item) && item.data){					
					_.map(item.data, function(d, k){
						var temp = _.pick(d, ['id', 'stock', 'qty_model']);
						//in case of bundel product set qunatity zero
						if(prd_type!== undefined && prd_type =="bundle"){
							d.qty_model =0;
						}
						
						//check product in stock or not
						if(temp.stock!== undefined && temp.stock == "true"){
							temp.sold_out = false;
						}else{
							temp.sold_out = true;
						}

						prd_attr_selection.push(temp);
					});				
				}	
			});
		}
	...
	function productDetailHandler($scope,salesfactoryData,$filter,$window,$timeout,$http,$location,$anchorScroll,$rootScope,$sce, uiGridConstants, dataManipulation){
		...
		*/
					...
					switch(item.prdType){
						case 'normal':
							if(item.totQty == 0 || item.totQty == ""){
								...
								item.sold_out = true;
								item.selected = false;
								prd_attr_selection.push({'id' : item.id, 'qty_model' : item.qty_model, 'sold_out' : item.sold_out});
							}else{
		...
		//console.log(dataManipulation.setProps(productData.productInfo));
		...
		*==============================*/		
		...
		//prdSelectionData(productData.tableView, productData.oneProductInfo.prdType);
		dataManipulation.costructDataTabular(productData.tableView, productData.oneProductInfo.prdType, productData.productInfo);
