	...
	function _createThumbVideoHtml(dataImg,dataVid,strflag){
		...
		if(!angular.isUndefined(dataImg) && dataImg.length>0){
			angular.forEach(dataImg,(item,index)=>{
				...
				thumbHtml+= "<a data-slide-id='zoom' data-zoom-id='Zoom-1' href='"+item.large+"'  data-image='"+item.original+"' onclick='thumbimagevedio(event,\"thumbimage\")'> <img src='"+item.thumb+"' alt='"+item.image+"' /></a>";
		   });
		}
