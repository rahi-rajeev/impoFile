                                ...
                                <div class="size col-sm-6" data-ng-repeat="atr in rvCtrl.attrRes[prdDetails.mainProductId] track by $index">
                                    ...
                                    <span class="custom-select block" data-ng-if="atr.attribute.attribute_type_value == 'text_color_image'">
                                       <div class="pdetail-row color-choose"> 
                                            <ul id="colorVariant">
                                                <li ng-repeat="attrname in rvCtrl.attrValRes[prdDetails.mainProductId][atr.attribute_id] track by attrname.valId" uib-popover-html="rvCtrl.prdTooltip" popover-trigger="'mouseenter'" data-ng-mouseover="rvCtrl.getTooltipContent(attrname.color_image,attrname)">
                                                    <span class="select-color">
                                                    <input type="radio" name="color" id="attributeValueId[]" data-ng-model="rvCtrl.selAttrVal[prdDetails.mainProductId][$parent.$index].valId" data-ng-click="rvCtrl.changeContent($parent.$index,atr.attributedesc.attr_id,prdDetails.mainProductId,prdDetails.id,$parent.$parent.$parent.$index)" data-ng-value="<%attrname.valId%>" class="multiSelectRadioBtn">
