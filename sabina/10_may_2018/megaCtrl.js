(function() {
   ...
   function _megaMenuHandler($scope, salesfactoryData, $timeout) {
      ...
      $scope.iconLibrabry = [{
         ...
         "icons": ["file-archive-o", "file-audio-o", "file-code-o", "file-excel-o", "file-image-o", "file-movie-o", "file-o", "file-pdf-o", "file-photo-o", "file-picture-o", "file-powerpoint-o", "file-sound-o", "file-text", "file-text-o", "file-video-o", "file-word-o", "file-zip-o"]
      }, {
      ...
      $scope.systmePageLinks = [{
         ...
         {
            'title': 'Listing Page',
            'url': $scope.site_url + '/category/lorem-ipsum'
         },
         {
            'title': 'Product Detail Page',
            'url': $scope.site_url + '/product/panty/MC1509002649FG'
         },
         {
            'title': 'Cart Page',
            'url': $scope.site_url + '/cart'
         },
         {
            'title': 'Checkout Page',
            'url': $scope.site_url + '/cart'
         },
         {
            ...
            'url': $scope.site_url + '/thanks'
         },
         {
            'title': 'Login Page',
            'url': $scope.site_url + '/login'
         },
         {
      ...
      $scope.megaMenuTreeOption = {
         accept: function(sourceNode, destNodes, destIndex) {
            var modelValue  = sourceNode.$modelValue;
            var type = destNodes.$parent.$type;
            console.log("start=======")
            console.log(sourceNode, destNodes, destIndex);
            console.log(modelValue);
            console.log(destNodes.$parent);
            console.log("end========")
            return true
            if(type === "uiTreeNode" && type!=="uiTree")
              return true;
            else if(type !== "uiTreeNode" && type ==="uiTree")
                return true;
            else return false;
         },
...
*/
      ...
      _getImagesList = function(typeId) {
         salesfactoryData.getData($scope.imagesListUrl, 'GET', {
               type: typeId
            })
            .then(function(r) {
               if (typeId == 'images') {
      ...
      $scope.saveMenu = function() {
         if ($scope.menu.title == '') {
            ...
            $('#msg').hide();
            $('#msg').addClass('');
            salesfactoryData.getData($scope.saveUrl, 'POST', {
                  menu: $scope.menu,
                  data: $scope.data,
                  id: $scope.id
               })
      ...
      _getMegaMenuData = function() {
         salesfactoryData.getData($scope.geturl, 'GET', {
               'id': $scope.id
            })
            .then(function(r) {
               //response section
      ...
      _getStaticPageList = function(typeId) {
         salesfactoryData.getData($scope.typelistUrl, 'GET', {
               type: typeId
            })
            .then(function(r) {
               if (typeId == 4) {
      ...
      _getBlockTypeList = function(typeId) {
         salesfactoryData.getData($scope.blockListUrl, 'GET', {
               type: typeId
            })
            .then(function(r) {
