...
<script type="text/javascript">
   ...
   var form_action_url = "{{ action('Admin\Blogs\BlogController@update') }}";
   ...
   //for form field name 
   var formFieldName = ["category","cms_url","status","features","comment","publish","blog_title","blog_heading","blog_desc","tagss","meta_title","meta_keyword","meta_desc"];
   var errorMsg = ["Please select category", "Please enter url", "Please select status", "Please select feature", "Please select comment","Please select publish","Please enter blog title","Please enter bloag heading","Please enter blog description","Please enter blog tag","Please enter blog meta title","Please enter blog meta keyword","Please enter blog meta description"];
   var previousData = {!! $page_dtls !!};  
   var image_base_url = {!! $image_base_url !!};
   var deleteBlogImage = "{{ action('Admin\Blogs\BlogController@deleteBlogImage') }}";
...
@section('content')
    <div class="content" ng-app="blogApp" ng-controller="blogAddEditCtrl" ng-cloak>
        ...
        @endif 
        <form id="cmsForm" name="cmsForm" class="form-horizontal form-bordered" enctype="multipart/form-data" novalidate>
            <div class="header-title clearfix">
                
                <h1 class="title">@lang('cms.edit_page') : @if(isset($page_dtls->blogDesc->blog_title)) {{$page_dtls->blogDesc->blog_title}} @else {{'N/A'}} @endif</h1> 
            ...
            <div class="content-wrap">
                ...
                <div class="content-right container">
                    ...
                    {{ csrf_field() }}
                    {{ method_field('PUT') }}
                    <input type="hidden" name='blog_id' value="{{$page_dtls->id}}">
                            ...
                            <div class="form-row row">
                                <div class="col-md-5">
                                 <label>@lang('common.thumb_image') <i class="strick">*</i></label>                                 
                                    <div>
                                        <img src="{{ getBlogImageUrl($page_dtls->image,'blog-570x402') }}" width="276">
                                    </div>
                                    <div>
                                    ...
                                    <div class="multiple-file-upload">
                                        ...
                                        <div class="table table-striped files">
                                           <droplet ng-model="blog.file_interface">
                                            <ul class="files">
                                                <li ng-repeat="file in blog.prevImage">
                                                    <div class="col-sm-2">
                                                     <img ng-src="<%blog.image_base_url%><%file.image%>" alt="<%file.image%>" width="90">
                                                    </div>
                                                    <div class="col-sm-6">
                                                       <p>
                                                        <input name="default_image" type="radio" value="<%$index%>" ng-model="blog.prevDefaultImage"  ng-change="addCheckAttrInItem(file,'prevImg',$index)"/> 
                                                        @lang('product.set_as_display_img')
                                                       </p>
                                                    </div>
                                                    <div class="col-sm-2">
                                                       <div class="secondary cancel" ng-hide="file.is_default==1"  ng-click="deletePrevUploadImg($event,file.id,$index)">
                                                ...
                                                <li ng-repeat="item in blog.file_interface.getFiles(blog.file_interface.FILE_TYPES.VALID)">
                                                    ...
                                                    <div class="col-sm-6">
                                                       ...
                                                       <p>
                                                         <input name="default_image" type="radio" ng-model="blog.defaultImage" value="<%$index%>" ng-change="addCheckAttrInItem(item,'dropImg',$index)"> 
                            ...
                            <h2 class="title-prod">
                             ...
                             <div class="form-row row">
                                ...
                                <label>@lang('cms.url_key') <i class="strick">*</i></label> 
                                    ...
                                    <input type="text" name="cms_url" ng-model="blog.cms_url" required>
                                    @if ($errors->has('url'))
            ...
            <input type="hidden" name="productids" name="" id="assigned_product_ids">
        </form>      
    </div>
