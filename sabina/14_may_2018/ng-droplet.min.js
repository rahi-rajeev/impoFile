(function($angular) {
        ...
        return {
            ...
            controller: ['$scope', function DropletController($scope) {
                ...
                $scope.options = {
                    ...
                    /**
                     * @property maxNumberFile (number of file user can upload)
                     * @type {Number|Infinity}
                     * @default Infinity
                     */
                    maxNumberFile : Infinity,
                };
                ...
                (function createModelBlueprint() {
                    ...
                    $scope.DropletModel.prototype = {
                        ...
                        load: function load(file) {
                            ...
                            this.maximum = file.maximum || $scope.options.maximumValidFiles;
                         ...
                         * @return {void}
                         */
                        setType: function setType(type,prdId,prdIndex,optId,mainPrdId) {
                            this.type = type;
                            if(typeof prdId!=='undefined' && prdId!='')
                                this.productId = prdId;
                            if(typeof prdIndex!=='undefined' && prdIndex!='')
                                this.productIndex = prdIndex;
                            if(typeof optId!=='undefined' && optId!='')
                                this.optionId = optId;
                            if(typeof mainPrdId!=='undefined' && mainPrdId!='')
                 ...
                 * @param file {File}
                 * @param type {Number}
                 * @param prdId,prdIndex {optional}
                 * @param maximum {optional}
                 * @return {Object}
                 */
                $scope.addFile = function addFile(file, type,prdId,prdIndex,optId,mainPrdId,maximum) {
                    // If the developer didn't specify the type then we'll assume it's a valid file
                    // that they're adding.
                    type = type || $scope.FILE_TYPES.VALID;
                    maximum = maximum || $scope.options.maxNumberFile;
                    
                    // Create the model and then register the file.
                    var model = new $scope.DropletModel();
                    model.setType(type,prdId,prdIndex,optId,mainPrdId,maximum);
                 ...
                 * @return {void}
                 */
                $scope.traverseFiles = function traverseFiles(files,prdId,prdIndex,optId,mainPrdId) {
                   for (var index = 0, numFiles = files.length; index < numFiles; index++) {
                        ...
                        $scope.addFile(file, type,prdId,prdIndex,optId,mainPrdId,maximum);
                ...
                (function setupDirectiveInterface() {
                    ...
                    $scope.interface = {
                        ...
                        /**
                         * @method setMaxNumberFiles
                         * @param value {Number}
                         * @return {void}
                         */
                        setMaxNumberFiles: function setMaxNumberFiles(value) {
                            $scope.options.maxNumberFile = Number(value);
                        },
    ...
    (function createInputElements() {
                ...
                return {
                    ...
                    link: function link(scope, element,attr) {                                         
                        ...
                        element.bind('change', function onChange() {
                            scope.$apply(function apply() {
                               scope.interface.traverseFiles(element[0].files,scope.productId,scope.productIndex,scope.optionId,scope.mainPrdId);
                            });
                        });
        ...
        createDirective('dropletUploadSingle', '<input class="droplet-upload droplet-single" type="file" accept="image/*" />');
        createDirective('dropletUploadMultiple', '<input class="droplet-upload droplet-multiple" type="file" multiple="multiple" accept="image/*" />');
