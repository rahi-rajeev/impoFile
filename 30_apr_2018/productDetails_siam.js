...
(function(){
	...
	*/
	function columnSetting(colsDef){		
		...
		_.map(colsDef, function(val, key){
			...
			if(val.field!== undefined && val.field!=='id'){
				...
				switch(val.field){
					case 'initial_price': 
					    colObj['cellTemplate'] = '<div class="price-wrap"> <span class="old-price"  data-ng-if="row.entity.special_price!=\'0\'">'+curCode+'&nbsp;<%row.entity.initial_price%></span>'+
					    	'<span ng-class="{\'new-price\':row.entity.special_price!=\'0\'}" >'+curCode+'&nbsp;<%row.entity.special_price%> </span></div>';
					   	break;
					case 'qty':
						colObj['cellTemplate'] = " <span class=\"spiner\">"+
                			...
                			"<input class=\"spinNum input-text qty\" max=\"<% row.entity.qty %>\" min=\"1\"  ng-model=\"row.entity.qty_model\"  value=\"<% row.entity.qty_model %>\" ng-blur=\"grid.appScope.rvCtrl.tableView.incDcrQuntity($event, row.entity.qty, 'tqchange', row.entity, rowIndex)\" type=\"text\" id=\"quantity\" name=\"quantity\" ng-pattern=\"/^\d+$/\">"+
	...
	/*
	*@description: This function used for tabular product data selection object
	*@param : prd_attr_selection {object} ->store data main product wise
	*/
	var prd_attr_selection = {};

	function prdSelectionData(obj){
		if(obj!== undefined && obj){
			_.map(obj, function(item, key){
				//in case of config nad bundel
				if(!_.isArray(item) && item.data){
					var name = key.toString();
					prd_attr_selection[name] = [];
					_.map(item.data, function(d, k){
						prd_attr_selection[name].push(_.pick(d, ['id', 'stock', 'qty_model']));
					});				
				}	
			});
		}
	...
	//controller start =================================*/
	function productDetailHandler($scope,salesfactoryData,$filter,$window,$timeout,$http,$location,$anchorScroll,$rootScope,$sce, uiGridConstants){
		...
		//view setting (list or table->grid)
		rvCtrl.productLayoutView = 'grid';	
		/*
		*@ngdoc
		*descriptions : Self execute function for set total quantity model, set product info (with main product id and type).
		*Check all product quantity for enable sold out button
		*@param : prdDataObj {array}
		*/
		function setGetCustomProperty(prdDataObj){
			_.forEach(productData.productInfo,function(item,index){
				if(item.totQty!==undefined && !isNaN(item.totQty)){
					switch(item.prdType){
						case 'normal':
							var name = item.id.toString();
							prd_attr_selection[name] =[];

							if(item.totQty == 0 || item.totQty == ""){
								rvCtrl.totalQtyVal.push(0);
								item.qty_model = 0;
								item.sold_out = true;
								item.selected = false;
								prd_attr_selection[name].push({'id' : item.id, 'qty_model' : item.qty_model, 'sold_out' : item.sold_out});
							}else{
						...
						case 'configrable':
							if(item.totQty == 0 || item.totQty == ""){
								...
								if(item.min_quantity!== undefined && !isNaN(item.min_quantity)){
									rvCtrl.totalQtyVal.push(item.min_quantity);
									item.qty_model = item.min_quantity;
									item.sold_out = false;
									item.selected = false;
								}else{
									rvCtrl.totalQtyVal.push(1);
				...
				var attr_count = productData.attrres[item.mainProductId] ;
				
				prd_info.push(
					{"mainprdid" : item.mainProductId,
					 "type" : item.prdType, 
					 "product_attribute_count" : attr_count.length,
					 "product_type" : productData.oneProductInfo.prdType,
					}
				); 
		...
		setGetCustomProperty(productData.productInfo);
		...
		*@props : prd_attr_selection {object} -> for product selction array on selection
		*==============================*/		
		...
		prdSelectionData(productData.tableView);
		...
		*/ 
		...
		function constructProdSeclectionData(mainPrdId, row){			
			var ind =  _getIndex(prd_attr_selection[mainPrdId], row.entity.id, 'id');

			if(ind== -1 && row.isSelected){
				prd_attr_selection[mainPrdId].push({'id' : row.entity.id, "selected" : row.isSelected, 'quantity' : row.entity.qty_model});
			}else if(ind >=0){
				if(row.isSelected){
					prd_attr_selection[mainPrdId][ind].selected = row.isSelected
				}else{
					prd_attr_selection[mainPrdId].splice(ind,1);
				}				  
			}	
		};

		//register the API (required for getting the selected rows)		
		$timeout(function(){
			registerGridApi = function(gridApi){
				console.log('registerGridApi(gridApi);');
				//set gridApi on scope
				rvCtrl.gridApi = gridApi;

				//trigger on row selection change
				gridApi.selection.on.rowSelectionChanged($scope, function(row){
				    constructProdSeclectionData("5217", row);				 
				});

				//trigger on batch selection change
			...
			registerGridApi(rvCtrl.gridApi);
		});
		...
		rvCtrl.tableView = {
			...
			*/
			...
			addToCartHandler : function($event,strflag){
				...
				**/
				function _checkAttrSelected(){
					...
					_.map(prd_info, function(item, index){
						if(item.product_type!== undefined && (item.product_type === "bundle" || item.product_type === "configrable")){
							if(item.type==="normal"){
								//
							}else if(item.type==="configrable" || item.type==="bundle"){
								var x = _.filter(prd_attr_selection[item.mainprdid], {selected : true});
								//check selection
								if(x.length){
									//check qunatity
									var qt = _.filter(x, {qty_model : 0});									
									if(qt.length) gotocart = "no"; 										
								}								
							}
						}
						// 	}else if(item.product_type!== undefined && item.product_type === "normal"){
			...
			getPriceByOption : function($event,str,prdDetail,index){
				console.log(prdDetail);
				if(str==="tqchange" && prdDetail.qty_model ==0 ) return;
				loaderImage('show');
				var dataObj = {
					'productId' : prdDetail.id,
					"quantity" : prdDetail.qty_model,
					"optionValueId" : '',
					"optionId" : ''
				};
			...
			incDcrQuntity : function($event,valMax,str,prdDetail,index){
				...
				if (str == "up" && tq < valMax) {					
					...
					if(!isNaN(tq) && tq > valMax){
	      			...
	      			this.getPriceByOption($event, str, prdDetail.id, index);	
				}
			},
