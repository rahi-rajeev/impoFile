	...
	function productCtrl($scope,salesfactoryData,$http,$sce,$timeout,$filter, treeNodeService,$rootScope){
			...
			if(!angular.isUndefined(product_detail) && angular.isObject(product_detail)){
				...
				if(product_detail.sp_tp_bp_type == 1){
					...
					if(product_detail.from_date !== "0000-00-00"){
						$scope.product.sp_fromdate = moment(new Date(product_detail.from_date)); 
					}			
					if(product_detail.to_date !== "0000-00-00"){
						$scope.product.sp_todate = moment(new Date(product_detail.to_date));
					}							
				...
				if(product_detail.nexpiry == 1){
					...
					if(product_detail.expiry_datetime!== "0"){
						$scope.product.expiry_date = moment(new Date(product_detail.expiry_datetime));
					}
				}
				...
				if(product_detail.product_unit!==undefined && product_detail.product_unit){
					$scope.product.product_unit = product_detail.product_unit;
				}
				...
				if(product_detail.package_unit!==undefined && product_detail.package_unit){
					$scope.product.package_unit = product_detail.package_unit;
				}
				...
				if(product_detail.product_weight_unit!==undefined && product_detail.product_weight_unit){
					$scope.product.weight_unit = product_detail.product_weight_unit;
				}
				...
				if(product_detail.total_weight_unit!==undefined && product_detail.total_weight_unit){
					$scope.product.total_weight_unit = product_detail.total_weight_unit;
				}
		...
		***/
		...
		$scope.switchPecialPrice =($event,spType)=>{
			...
			console.log($scope.product.stbprice);
			console.log('tire price');
			console.log($scope.product.tprices);
			console.log('bundel price');
			console.log($scope.product.bundelprice);*/
			// stbprice:'',
		        ...
		        if (isConfirm) {
		            ...
		            .then((response)=>{
		                if(response.data.status=='success') {
						   ...
						   var indx = _.findIndex($scope.product.productImages,(o)=>{
					...
					if(item.front_input==="multiselect" || item.front_input==="select"){
							...
							for(var i in _filerObj.option_attr_value_detail){
								...
								if(r.length>0){
									v[i] = {
										...
										"pricetype" : _filerObj.option_attr_value_detail[i].price_type.toString(),
						...
						if(_filerObj){
							$scope.product.reqmodel[index] = {
								...
								"pricetype" : (_filerObj.price_type ==1)? "Percent" :"Fixed",
		...
		*/		
		...
		$scope.processProductImages = (productObj) => {
			...
			.then(() => {
				...
				if(!isNaN(defaultImgIndx) && images.length > 0){
					...
					if(prd_defalut_img.dropImg!==undefined && prd_defalut_img.dropImg>=0){
						images[prd_defalut_img.dropImg].is_default = true;
					}else if(prd_defalut_img.prevImg!==undefined && prd_defalut_img.prevImg>=0){
						var temp  = angular.copy($scope.prevImage[prd_defalut_img.prevImg]);
						temp.isDefault = true;
						images= images.concat(temp);						
					}					
				}else{
					if(prd_defalut_img.prevImg!==undefined && prd_defalut_img.prevImg>=0){
						var temp  = angular.copy($scope.prevImage[prd_defalut_img.prevImg]);
						temp.isDefault = true;
						images= images.concat(temp);						
					}	
				}
				console.log('herefefefef');
		...
		$scope.processDates = (productObj) => {
			...
			if(productObj.expiry_date !== undefined && productObj.expiry_date){
				...
				// console.log(productObj.expiry_date.toISOString());
			...
			if(productObj.sp_todate !== undefined && productObj.sp_todate){
				...
				//productObj.sp_todate = productObj.sp_todate.toDate();
			...
			.then((productObj) => {	
				...
				$http({
					...
					transformRequest : function(){
						...
						formdata.append('product',angular.toJson(productObj));
						var defImg={};
				    	var def='';	
				    	...
				    	if($scope.product.prevDefaultImage!==undefined && $scope.product.prevDefaultImage!==""){
				    		...
				    		defImg["id"] = $scope.prevImage[ind].id;
				    		defImg["isDefault"] = true;
				    		defImg["name"]= $scope.prevImage[ind].image;
						...
						//for product image 						
						if(validDropletFiles !== undefined && validDropletFiles.length>0){
							for(var i=0; i<validDropletFiles.length; i++){
								//for default image
								if(def!== "" && def === i){
									defImg["id"]= "";
						    	  	defImg["isDefault"] = true;
						    	  	defImg["name"]= validDropletFiles[i].file.name;
						    	  	defImg["index"] = def;									
								}	
								formdata.append("product_images["+i+"]", validDropletFiles[i].file);  
							}							
						}
