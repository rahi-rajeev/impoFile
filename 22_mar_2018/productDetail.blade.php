...
<!-- Content section start -->
    <div class="productDisplay ng-cloak"  data-ng-app="productDetailApp" data-ng-controller="productDetailCtrl as rvCtrl" ng-cloak>
        ...
        <div class="recommend-product">
            ...
            <ul class="recomend-slider" id="related-product-section" ng-if="rvCtrl.related_prd_data" enable-slider>
                <li ng-repeat="item in rvCtrl.related_prd_data">
                    <a href="<%item.url%>">
                      <img ng-src="<%item.thumbnail_image%>" height="285" alt="">
                    </a>
                    <div class="product-info">
                        <h2 class="product-name"><%item.name%></h2>
                        <span class="sku"><%item.sku%></span>
                        <div class="price-wrap">
                            <span class="old-price"  data-ng-if="item.isDisc"><%item.initial_price%>  {{session('default_currency_code')}}</span>
                            <span ng-class="{'new-price':item.isDisc}" ><%item.special_price%> {{session('default_currency_code')}}</span>
                            <span class="discount-price"  data-ng-if="item.isDisc"><%item.isDisc%>%</span>
                        </div>
                    </div>
                </li>  
            </ul>
