(function(angular,undefined){
    ...
    function ctrlHendler($scope,$timeout,$window,$sce,$rootScope,salesfactoryData,uiGridConstants){
        ...
        *@begin_state ->used for store beginning table structure
        *@after_save_state -> used for store after save by user table structure
        */
        $scope.structure={
            reset_btn_enable : false,
            save_table_btn_enable : false,
            begin_state : [],
            after_save_state :{},
            store_colDef : [],
        };
        ...
        */
        function _getTableListData(url,type,page,per_page,filter_obj){
            let obj ={
                ...
                "action_type" :  (type!==undefined && type!='') ? type : data_cat,
        ...
        $scope.HeaderPagination = {
            ...
            nextPage : function() {
                if ($scope.gridOptions.paginationCurrentPage < this.getTotalPages()) {
                    ...
                    $scope.showLoaderTable = true;
                    let filetrObject = getObjectClone($rootScope.filedSetModel);
        ...
        *****/
        ...
        $scope.enableTab = function(str,ftInfo) {
            ...
            if (str!== undefined && (str === 'active' || str === "filter")) {
                ...
                console.log("here????");
                data_cat = (ftInfo!== undefined && ftInfo) ? ftInfo :  data_cat;
                _getTableListData('',data_cat,$scope.gridOptions.paginationCurrentPage,$scope.gridOptions.PageSize, filetrObject);              
            } 
            //for all tab click
        ...
        $scope.gridOptions.onRegisterApi = function(gridApi) {
            ...
            //event on column resize
            gridApi.colResizable.on.columnSizeChanged($scope,function(colDef, deltaChange){
                var _col = {
                    field : colDef.field,
                    name : colDef.name,
                    width : (colDef.minWidth!== undefined) ? colDef.minWidth : 50, 
                };
                //console.log(colDef);
                //console.log(colDef.field);
                var _ind = _getIndex($scope.structure.store_colDef, colDef.field, "field");
                //console.log(_ind);
                if(_ind == -1){
                    $scope.structure.store_colDef.push(_col);
                }                
            });           
        };
        ...
        */
        function _filterDataHandler(){
            try{
                ...
                if(angular.isArray($scope.filterTab) && $scope.filterTab.length>0){
                    let ftData =  angular.copy($scope.filterTab);
        ...
        $scope.filterActionHendler ={
            ...
            */
            save : function(btnCheck, action, obj){
                ...
                let index = _getIndexOfData($scope.filterObj.jsonForField, obj.id);
            ...
            */
            deleteFilter : function(obj, action){
                ...
                .then(function(rsp){
                    if(rsp.data.status!== undefined && rsp.data.status=== "success"){
                        let index =  _getIndexOfData($scope.filterTab, obj.id);
                        ...
                        let jsIndex = _getIndexOfData($scope.filterObj.jsonForField, obj.id);
                        if(jsIndex>=0){
            ...
            */
            optChange : function(fId){
                ...
                //@param jsonForField               
                let index = _getIndexOfData(fObj.jsonForField, fId);
                //In case have already id in jsonForField then concat only value 
                if(index!== -1){
                    let valArr =  fObj.filterModel[fId];
                    //In case model value already in jsonForField set
                    let ind =  _getIndexOfData(fObj.jsonForField[index].value, valArr.id);
                    if(ind == -1){
            ...
            */ 
            removeField : function($event, _id, _index, _modelName, _modelNameTo){
                ...
                let ftabIndex = _getIndexOfData($scope.filterObj.jsonForField, _id);
        ...
        $scope.save_table_structure = function(event, flag){
            ...
            if(flag!== undefined && flag === "save"){
                ...
                console.log(_st.store_colDef);
                //in case columns resize then save current width
                if(_st.store_colDef.length){
                    _st.store_colDef.map(function(cl){
                        var _clind = _getIndex(_st.after_save_state.columns, cl.field, "name");
                        if(_clind!= -1){
                            cl.width = _st.after_save_state.columns[_clind].width;
                        }                       
                    });
                }
                console.log(_st.after_save_state);
                console.log(_st.store_colDef);
                ...
                //reset structure
                $scope.gridApi.saveState.restore($scope, _st.after_save_state);              
            }
    ...
    function _getIndexOfData(sourceObj, destObj){
        let index = sourceObj.findIndex(function(o){
